...
// ########### SweetAlert2 Icons: success: error, warning, info, question ################ //
	...
	addProductToCart: function() {
		...
		function btn_activate_add_cart(){

			if($(window).width() <=600){
					$('.products-card-rounded').each(function(){
				...
				/*if(!$('.products-card-rounded').hasClass('raise')){
						$('.products-card-rounded').each(function(){
					$(this).toggleClass('raise');
								});
				}*/
		...
		if(window.location.href.includes('product')){
			$('.products-card-wrapper').attr('style', 'background:none !important; background-color:white !important; border-radius: 30px !important; margin-bottom: 35px; border: 1px solid #E5E5E5;');
			
		}


		btn_activate_add_cart();
		...
		setTimeout(btn_activate_add_cart,50);
		...
		$('.category').children().children().children().off("click").on('click', function(){
			setTimeout(btn_activate_add_cart,50);
		});

		$('.mdi-chevron-left').parent().css('background-color', 'white');
		...
		$('.btn-modal-product-specification').click(function () {
			...
			var frameWin = getIframeWindow(el);
	...
	showSocialMediaIcons: function() {
		$.getJSON(querybiz.CONST_APIUrl + '/api/getSocialMedia', function(data) {
			...
			k.forEach(function(key) {
				...
				html += '<a href="' + href + '" target="_blank" title="' + querybiz.trans('Click to go to ' + key.toLowerCase() + ' Profile') + '"><img src="' + src + '" style="width: 0; margin-right: 2px; display: none"></a>';
			});
	...
	errorMsg: function (arMsg, arErrorMsg = null, messageContainer = null) {
		...
		if (arMsg.return === 'error' && arMsg.hasOwnProperty('msg')) {
			querybiz.showFormMsg(arMsg.msg, selfArErrorMsg, messageContainer);
		}
	},

	showFormMsg: function(arMsg, arErrorMsg, messageContainer = null) {
		let duration = 3000;
		let html = '';
		let maxTime = 0;
...
var querybizProduct = {
	...
	listenerBtnCartProductQuantityUpdate: function() {
		$('#btn_cart_products_quantity_update').on('click', function() {
	...
	viewCart: function() {
		this.post('/getCart', function(data) {
			...
			$.each(data.listProduct, function (key, val) {
				html += '<li>' + val.name + ' <input type="text" class="cart-products-quantity" data-product-id="' + val.id + '" value="' + data.productQuantity[val.id] + '" style="width: 30px; text-align: center"> => ' + data.productPriceQuantity[val.id] + ' (<a href="javascript:;" class="btn_cart_products_remove" data-product-id="' + val.id + '">Remove</a>)</li>';
			});

			html += '</ul>';
			if (data.listProduct.length) {
				html += '<BR><button id="btn_cart_products_quantity_update">Update</button> <button id="btn_cart_products_clear">Clear All</button>';
			} else {
			...
			$('#btn_cart_products_quantity_update').unbind();
			$('#btn_cart_products_quantity_update').click(function() {
			...
			$('.btn_cart_products_remove').unbind();
			$('.btn_cart_products_remove').click(function() {
				let productId = $(this).attr('data-product-id');
				querybizProduct.removerProductFromCart(productId);
			})
	...
	cartQuantityUpdate: function(callback = '') {
		...
		querybizProduct.post('/sendQuantityToCart?id=' + strId + '&quantity=' + strQuantity, function(data) {
	...
	removerProductFromCart: function(id) {
	...
	delProductFromCart: function () {
		$('.btn-del-product-cart').unbind();
		$('.btn-del-product-cart').click(function(evt) {
			var btnSelf = $(this);
			btnSelf.prop('disabled',true);
	...
	showFormMsg: function(msg) {
		let elMsgContainer = $('#spf_message');
		querybiz.showFormMsg(msg, elMsgContainer);
	},
		...
		$.ajax({
			...
			success: function(data) {
				...
				if (typeof(data) == 'object') {
					...
					showFormMsg('error-data-type');
				}
			},
			error:function(data) {
				showFormMsg('error-ajax-post');
				$("body").removeClass("form_disabled");
			}
		});
	},
...
var querybizCustomerDrive = {
	...
	trackUploadProgress: function (e) {
		if (e.lengthComputable) {
			...
			querybizCustomerDrive.progressBar.width(currentProgress + '%');
	...
	uploadFile: function(e) {
		...
		$.ajax({
			...
			success: function(data) {
				...
				querybiz.showFormMsg(arMsg);
			},
			error: function() {
				querybiz.showFormMsg(querybiz.trans('Error Gettings List of Files'));
			},
	...
	updateProfilePicture: function() {
		...
		if (data.return === 'success') {
			if (!data.error.length) {
				let filename = querybiz.CONST_apiUrl + '/data/customer/' + data.success;
...
var querybizCustomerMyAccount = {
	...
	isValidPtTin:function(form) {
		...
		let invalid = '<span class="not-valid required text-danger small float-right">' + this.CONST_invalidTinMsg + '</span>';
...
var querybizCustomerSupport = {
	...
	supportDetailsData: function(data) {
		...
		$('.btn-support-ticket-close').click(function() {
			...
			if (supportStatus == 'closed') {
				querybiz.showFormMsg(querybiz.trans('This Support Ticket is already closed') + '!');
			} else {
	...
	supportDetailsHistory: function(details, historyList, historyLength) {
		...
		$.each(historyList, function (key, val) {
			...
			html += '   <div class="d-inline-block p-1 text-left" style="width: 150px; height: 30px">' + val.date + '</div>';
...
var querybizCustomerReview = {
	...
	init: function (options) {
		...
		$('textarea[name=observations]').bind('keyup', function(e) {
			...
			let html = charsTotal < minTotal || charsTotal >= maxTotal
				?
				'<sup class="text-danger">'+charsTotal+'</sup>'
				:
				'<sup class="text-success">'+charsTotal+'</sup>';
		...
		$('.set-image').change(function(e) {
			let form = $(this).closest('form');
			let html = e.target.files[0] != undefined
				?
				'<img class="output" style="max-height:36px" src='+URL.createObjectURL(e.target.files[0])+'>'
				:
