var querybizCustomerMyInvoices = {
    ...
    init: function (options) {
        ...
        querybizCustomerMyInvoices.CONST_invalidTinMsg = options.hasOwnProperty('invalidTinMessage') ? options.invalidTinMessage : '';
        ...
        // 177 is Portugal, disable inputs and enable selectcs
        $('.geo-countries').change(function(){
            form = $(this).closest('form')
            if($(this).val() == 177){
                $(form).find('input.geo-pt-council, input.geo-pt-district').addClass('d-none').attr('disabled', true);
                $(form).find('select.geo-pt-council, select.geo-pt-district').removeClass('d-none').removeAttr('disabled');
            }
            else{
    ...
    isValidPtTin:function(form){
        ...
        invalid = '<span class="not-valid required text-danger small float-right">'+this.CONST_invalidTinMsg+'</span>';
